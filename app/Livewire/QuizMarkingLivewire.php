...
class QuizMarkingLivewire extends Component
{
    use WithPagination;
    ...
    {
        return view('livewire.quiz-marking-livewire', [
            'answers' => SubmitQuiz::with(['optionstexts', 'quiz', 'student'])->where('user_id', auth()->user()->id)->paginate(3),
            'submitToStudents' => Student::where('department_name', 'COMPUTER SCIENCE AND ENGINEERING DEPARTMENT')->get()
        ]);
    }
